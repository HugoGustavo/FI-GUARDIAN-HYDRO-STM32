...
typedef struct at_cmd_reset_t{
} at_cmd_reset_t;

typedef struct at_cmd_echo_t{
	uint8_t enabled;
} at_cmd_echo_t;

typedef struct at_cmd_mux_t{
    uint8_t enabled;
} at_cmd_mux_t;

typedef struct at_cmd_wifi_autocon_t{
    uint8_t enabled;
} at_cmd_wifi_autocon_t;

typedef struct at_cmd_wifi_mode_t{
    uint8_t ap_station;
} at_cmd_wifi_mode_t;

typedef struct at_cmd_wifi_ap_dhcp_t{
    uint8_t ap_station;
    uint8_t enabled;
} at_cmd_wifi_ap_dhcp_t;

typedef struct at_cmd_wifi_ap_net_cfg_t{
    const uint8_t*	ip;
    const uint8_t*	gateway;
    const uint8_t*	mask;
} at_cmd_wifi_ap_net_cfg_t;

typedef struct at_cmd_wifi_ap_dhcp_cfg_t{
    uint8_t 		enabled;
...
typedef struct at_cmd_wifi_ap_security_t{
    const uint8_t* 	ssid;
    const uint8_t* 	pwd;
    uint8_t 		channel;
    uint8_t 		crypto;
...
typedef struct at_cmd_con_close_t{
    uint8_t channel;
} at_cmd_con_close_t;

typedef struct at_cmd_con_server_t{
    uint8_t 	create;
    uint16_t 	port;
} at_cmd_con_server_t;

typedef struct at_cmd_con_server_tmrout_t{
    uint16_t timeout_s;
} at_cmd_con_server_tmrout_t;

typedef struct at_cmd_wifi_ap_set_t{
    const uint8_t*	ssid;
    const uint8_t* 	pwd;
} at_cmd_wifi_ap_set_t;

typedef struct at_cmd_con_start_t{
    const uint8_t* 	site;
...
typedef struct at_cmd_con_send_t{
    uint16_t 		size;
    int8_t 			channel;
    const uint8_t*	site;
    uint16_t 		port;
...
typedef struct at_cmd_ssl_buffer_t{
    uint16_t size;
} at_cmd_ssl_buffer_t;

typedef struct at_cmd_con_mode_t{
    uint8_t mode;
} at_cmd_con_mode_t;

typedef struct at_cmd_uart_t{
    uint32_t 	baud_rate;
    uint8_t 	data_bits;
    uint8_t 	stop_bit;
    uint8_t 	parity;
