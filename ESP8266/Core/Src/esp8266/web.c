...
static bool web_wait_response(uint8_t* channel, uint16_t* bytes){
    ...
    int16_t ans = ceb_cbuf_find_multi_str(at_get_cbuf(), web_channel_data, 5, 5000);
    if(ans == -1){
    	...
    	uint16_t buf_len = PAYLOAD_MAX_BUF-1;
        ans = ceb_cbuf_get_str_until_find(at_get_cbuf(), web_buffer, &buf_len, (uint8_t*)":", 1, 5000);
        if(buf_len < 4) ceb_cbuf_flush(at_get_cbuf());

        int c, b;
        web_buffer[buf_len] = '\0';
        if(sscanf((char *)web_buffer, "%d,%d:", &c, &b) != 2) ceb_cbuf_flush(at_get_cbuf());
        else{
            *channel = (uint8_t) c;
            *bytes = (uint16_t) b;
            ret = true;
...
bool web_client(void){
    ...
    if(web_wait_response(&channel, &bytes)){
        ...
        if(ret_code == 204) return true;
        if(ret_code / 100 == 2) return web_get_payload(bytes);
